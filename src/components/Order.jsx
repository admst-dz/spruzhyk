...
export const Order = ({ onBack }) => {
    ...
    return (
        <div className="w-full min-h-screen bg-[#E5E5E5] font-zen flex flex-col animate-fade-in overflow-y-auto text-black">

            <div className="p-6 md:p-10 flex items-center">
                <button onClick={onBack} className="flex items-center gap-2 px-6 py-2 bg-white rounded-full shadow-md text-sm font-bold text-black hover:scale-105 transition-transform">← Вернуться</button>
            </div>
            ...
            <div className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full px-4 md:px-10 pb-20 gap-10 md:gap-20">
                ...
                <div className="w-full md:w-1/3 flex flex-col gap-6">
                    <h2 className="text-3xl font-black text-black">ВАШ ЗАКАЗ</h2>
                    ...
                    <div className="bg-white p-6 rounded-[20px] shadow-xl flex flex-col gap-6">
                        ...
                        <div className="flex gap-4 h-64">
                            <div className="flex-1 rounded-[10px] shadow-inner relative overflow-hidden transition-colors duration-500" style={{ backgroundColor: coverColor }}>
                                ...
                                <div className="absolute bottom-2 left-2 text-white/60 text-[10px] font-bold">ОБЛОЖКА</div>
                                {bindingType === 'spiral' && (
                            ...
                            <div className="flex-1 bg-white border border-gray-200 rounded-[10px] relative flex items-center justify-center">
                                ...
                                <div className="absolute bottom-2 left-2 text-black text-[10px] font-bold">БЛОК</div>
                            </div>
                        </div>

                        {/* Текст строго черный */}
                        <div className="space-y-3 border-t border-gray-200 pt-4 text-sm font-medium text-black">
                ...
                <div className="w-full md:w-2/3 flex flex-col gap-6">
                    <h2 className="text-3xl font-black text-black">ОФОРМЛЕНИЕ</h2>
                    ...
                    <div className="bg-white p-6 md:p-10 rounded-[20px] shadow-xl flex flex-col gap-8">
                        ...
                        <div className="bg-gray-100 p-1 rounded-[12px] flex">
                            <button onClick={() => setClientType('phys')} className={`flex-1 py-3 text-sm font-bold uppercase tracking-widest rounded-[10px] transition-all ${clientType === 'phys' ? 'bg-white shadow-md text-black' : 'text-gray-500'}`}>Физическое лицо</button>
                            <button onClick={() => setClientType('jur')} className={`flex-1 py-3 text-sm font-bold uppercase tracking-widest rounded-[10px] transition-all ${clientType === 'jur' ? 'bg-white shadow-md text-black' : 'text-gray-500'}`}>Юридическое лицо</button>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 md:items-end justify-between border-t border-gray-100 pt-6">
                            <div className="flex flex-col gap-2">
                                <span className="text-sm font-bold text-black uppercase">Количество</span>
                                <div className="flex items-center gap-4 bg-gray-50 rounded-[12px] p-2 border border-gray-200 w-max">
                                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-8 h-8 bg-white text-black rounded-lg shadow-sm font-bold">-</button>
                                    <span className="text-xl font-bold w-8 text-center text-black">{quantity}</span>
                                    <button onClick={() => setQuantity(quantity + 1)} className="w-8 h-8 bg-white text-black rounded-lg shadow-sm font-bold">+</button>
                                </div>
                            </div>
                            {clientType === 'jur' && (
                                <label className="flex items-center gap-3 cursor-pointer group bg-blue-50 px-4 py-3 rounded-[12px] border border-blue-100 hover:bg-blue-100 transition">
                                    <input type="checkbox" checked={isSample} onChange={(e) => setIsSample(e.target.checked)} className="w-5 h-5 accent-blue-900 rounded cursor-pointer" />
                                    <div className="flex flex-col"><span className="font-bold text-blue-900 text-sm">Тиражный образец</span><span className="text-[10px] text-blue-600">Пробный экземпляр</span></div>
                                </label>
                            )}
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <button className="py-4 rounded-[12px] border-2 border-gray-200 text-black font-bold uppercase tracking-wider hover:border-gray-400 transition">Согласовать с менеджером</button>
                            <button className="py-4 rounded-[12px] bg-black text-white font-bold uppercase tracking-wider hover:bg-gray-800 transition shadow-xl">Оформить Заказ →</button>
                        </div>
                    </div>
                </div>
            </div>
...
// Черные лейблы и значения
const Row = ({ label, value }) => (<div className="flex justify-between items-center"><span className="text-gray-500 font-bold text-xs uppercase">{label}</span><span className="font-bold text-black">{value}</span></div>)
const ColorDot = ({ color }) => (<div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full border border-gray-300 shadow-sm" style={{ backgroundColor: color }} /><span className="uppercase text-xs font-bold text-black">{color}</span></div>)
const InputGroup = ({ label, placeholder }) => (<div className="flex flex-col gap-2"><label className="text-xs font-bold uppercase text-black ml-1">{label}</label><input type="text" placeholder={placeholder} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-[12px] text-black font-bold focus:outline-none focus:ring-2 focus:ring-black/20 placeholder-gray-400"/></div>)
